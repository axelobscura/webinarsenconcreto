              ...
              <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                {categorias.map((categoria) => {
                  if (categoria.tipo === 'superior') {
                    return (
                      <Link href={`/categorias/${categoria.url}`} key={categoria.id} className='flex items-center justify-center w-full p-2 text-center transition bg-white border border-white rounded-lg shadow-lg bg-opacity-20 hover:bg-opacity-50 hover:text-gray-900'>
              ...
              <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                {categorias.map((categoria) => {
                  if (categoria.tipo === 'consulta' && categoria.nombre !== 'Inteligencia Artificial') {
                    return (
                      <Link href={`/categorias/${categoria.url}`} key={categoria.id} className='flex items-center justify-center p-2 text-center transition bg-white border border-white rounded-lg shadow-lg bg-opacity-20 hover:bg-opacity-50 hover:text-gray-900'>
                        <h3 className='py-2 text-2xl font-bold leading-7 text-white hover:text-gray-950'>{categoria.nombre}</h3>
                      </Link>
                    )
                  }
